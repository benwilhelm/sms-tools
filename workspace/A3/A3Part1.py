...
def minimizeEnergySpreadDFT(x, fs, f1, f2):
    ...
    # get period (in samples) of two frequencies 
    p1 = fs / f1 
    p2 = fs / f2
    
    # generate lcm to determine shortest sample that contains integral
    # numbers of periods of each freq (M)
    M = (p1 * p2) / gcd(p1, p2)
    sample = x[:M]

    # generate DFT of sample
    f = fft(sample)
    
    # calc magnitude spectrum (abs) of positive half (M/2 + 1)
    spec = 20*np.log10(np.abs(f[:M/2+1]))
    return spec
    
    # return log scaled mag spec
